...
Unfortunately more parameters, in addition to the simple fact that every pair of images will have unique features, scales, and so on, means that the function we built was essentially personalized for our main test image. So, we generalized the disparity map function in our personal testing function to pass parameters that would optimize each stereo pair we were dealing with. 
