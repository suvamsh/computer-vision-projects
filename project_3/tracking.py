...
def get_initial_position(frame, hough_parameters):
    ...
    #circles = cv2.HoughCircles(frame, cv2.cv.CV_HOUGH_GRADIENT, 7, 1000,       WORKS FOR 1, 2, 4
    ...
    circles = cv2.HoughCircles(frame, cv2.cv.CV_HOUGH_GRADIENT, **hough_parameters)
...
def track_ball(video, hough_parameters, initial_radius_scalar = 1):
    ...
    r,h,c,w = int(initial_circle[0] - initial_circle[2]), int(initial_circle[2] * 2 * initial_radius_scalar), initial_circle[1] - initial_circle[2], int(initial_circle[2] * 2 * initial_radius_scalar)
